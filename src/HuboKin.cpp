...
namespace HK { 
  ...
  void HuboKin::armFK(Isometry3d &B, const Vector6d &q, int side, 
    ...
    vector<Isometry3d, Eigen::aligned_allocator<Isometry3d> > BVec(8);

    armFK(BVec, q, side, endEffector, -1, 6);
    B = BVec[7];    
  }


  int HuboKin::armFK(std::vector<Isometry3d, Eigen::aligned_allocator<Isometry3d> > &B, const Vector6d &q, int side, const Isometry3d &endEffector, int fromFrame, int toFrame) const {

    if (fromFrame > toFrame) {
      cout << "fromFrame must be less than or equal to toFrame." << endl;
      return 1;
    }
  
    B.resize(toFrame - fromFrame + 1);
    ...
    int j = 0; // Frame index counter
    if (fromFrame == -1) {
      B[j] = neck;
      fromFrame = 0;
    } else {
      B[j] = Isometry3d::Identity();
    }
    j++;
  ...
  void HuboKin::armJacobian(Matrix66d &J, const Vector6d &q, int side, const Isometry3d &location) const {

    Vector3d ee,oi,di,zi;
    Eigen::Matrix3d Ri;
    std::vector<Isometry3d, Eigen::aligned_allocator<Isometry3d> > B(8);
    ...
    armFK(B, q, side, hand, -1, 6); // Get transformation to each joint
    ee = B[7].translation(); // End-effector location
    for (int i = 0; i < 6; i++) {
      oi = B[i].translation(); // Joint i location
      di = oi - ee; // Vector from EE to Joint i
      Ri = B[i].rotation(); // Joint i orientation
      zi = Ri.col(2); // Joint i joint axis
      J.block(0,i,3,1) = di.cross(zi); // Column i of Jocabian
      J.block(3,i,3,1) = zi;
    }    
  }
